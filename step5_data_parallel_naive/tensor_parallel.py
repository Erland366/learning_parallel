...
# This is actually all-reduce btw
class Reduce(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        # Reverse of all reduce is copy (which I am not sure why it doesn't do it here)
        return grad_output
...
class Gather(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        # All CC operation must be contiguous
...
class Copy(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        if pgm.process_group_manager.tp_world_size == 1:
          return grad_output
        # Reverse of broadcast is all reduce
...
class RowParallelLinear(nn.Module):
    ...
    def forward(self, x):
        # The input here is already assumed chunked
